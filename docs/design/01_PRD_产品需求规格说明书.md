...
- MySQL FULLTEXT（`WITH PARSER n-gram`）直接查询实时数据，无需构建/更新索引
- 匹配逻辑：`MATCH ... AGAINST (BOOLEAN MODE)` 召回 + 逐词 `icontains` 子串过滤保证精度
- 查询前剥离 BOOLEAN MODE 操作符，避免用户输入的特殊字符导致语法错误
- 搜索结果截词高亮（自定义标签 `my_snippet`）

### 3.5 数据统计

- 文章浏览量统计（30 分钟按访客去重（Redis）+ 爬虫 UA 过滤，作者与超管访问不计数）
- 单页面浏览量统计（可装饰器注入，同样按访客去重）
...
| 便签笔记 | 简短笔记卡片瀑布流展示，支持搜索；点击卡片平滑放大查看全文，支持在线新增/编辑/删除 |
| 项目管理 | 个人项目展示列表 |
...
- **暗色主题**：支持深色模式切换
- **缓存策略**：Redis 缓存文章渲染结果（7 天）、RSS 正文、站点配置、侧边栏统计与未读通知计数；内容变更时按影响范围精确失效
- **性能**：渲染结果缓存、浏览量按访客 30 分钟去重、列表页分页（归档/标签/专题/时间线各 100 条）、列表与详情页消除 N+1
- **安全**：XSS 过滤（bleach）、评论内容清洗、CSRF 保护
- **可维护性**：Docker 部署，环境变量驱动配置

| — | 2026-07-24 | 根据 commit 2ca6b0b 更新：新增 3.8 脚本分享扩展模块 |
| — | 2026-09-05 | 根据 commit f7b1c7e 更新：便签笔记功能补充卡片放大查看与在线编辑能力描述 |
| — | 2026-09-11 | 根据 commit 292318d..2e20394 更新：3.4 搜索改为 MySQL FULLTEXT(n-gram) 现状（移除 haystack/Jieba/RealtimeSignalProcessor 描述）；浏览量统计改为「按访客 30 分钟 Redis 去重」；补充缓存策略与分页/N+1 性能描述 |
